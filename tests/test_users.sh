...
OUTPUT=$(curl -X GET "http://localhost:3000/auth?account=&pass=&user=" | \
...
OUTPUT=$(curl -X GET "http://localhost:3000/auth?account=a&pass=b&user=c" | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['code'])")
echo -e "\n TEST OUTCOME: "
if [ ${OUTPUT} -eq 3 ]; then
	echo -e "\t PASS \n"
...
OUTPUT=$(curl -X GET "http://localhost:3000/auth?account=${ACCOUNT}&pass=${PASS}&user=${USER}" | \
    python3 -c "import sys, json; print(json.load(sys.stdin)['code'])")
echo -e "\n TEST OUTCOME: "
if [ ${OUTPUT} -eq 0 ]; then
	echo -e "\t PASS \n"
